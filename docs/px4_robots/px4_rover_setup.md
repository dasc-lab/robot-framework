...
- in `EKF2_AID_MASK`: Check `GPS`, `vision position fusion`, and `yaw position fusion` if outdoor, and **only** `vision position fusion` and `yaw position fusion` if indoor
